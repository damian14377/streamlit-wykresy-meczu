...
def generate_charts(file_path, output_folder="output"):
    ...
    plt.figure(figsize=(12, 6))  # Większy rozmiar wykresu
    bars = plt.bar(df_sorted["Player Name"], df_sorted["Distance (km)"], color="blue")
    plt.ylabel("Dystans (km)")
    plt.suptitle(f"vs {session_title}", fontsize=18, fontweight='bold', y=1.02)  # Zmniejszenie wartości y dla lepszego widoku
    ...
    plt.grid(axis="y")
    plt.tight_layout(pad=3.0)  # Zwiększenie odstępu
